...
module UnbondedStaking.UnbondedPoolDebug (
...
stakeActLogic txInfoF paramsF purpose datum stakeActParamsF = unTermCont $ do
  ...
  pure . pmatch stakeActParamsF.maybeMintingAction $ \case
    ...
    PDNothing _ -> unTermCont $ do
      ...
      pguardC
        "stakeActLogic: failure when checking minted value in non-minting tx"
...
updateStakeLogic txInfoF paramsF spentInputF datum stakeAmt holderPkh = do
  ...
  pguardC
    "updateStakeLogic: update should not change rewards, totalRewards, totalDeposited, or next fields"
...
newStakeLogic txInfoF paramsF spentInputF datum holderPkh stakeAmt mintAct = do
  ...
  pure . pmatch mintAct $ \case
    PMintHead stateOutRef -> unTermCont $ do
      ...
      pguardC
        "newStakeLogic (mintHead): spent input does not match redeemer input"
        $ spentInputF.outRef #== pfield @"_0" # stateOutRef
      -- Validate next state
      pguardC
        "newStakeLogic (mintHead): next pool state does not point to new entry"
      ...
      pure . pmatch entryKey $ \case
        ...
        PDJust currentEntryKey' -> unTermCont $ do
          ...
          pguardC
            "newStakeLogic (mintInBetween): new entry's key should be strictly less than current entry"
            $ pfromData newEntryF.key #< currentEntryKey
          -- The new entry should point to the current entry
          pguardC
            "newStakeLogic (mintInBetween): new entry should point to current entry"
    ...
    PMintInBetween outRefs -> unTermCont $ do
      ...
      currEntryKey <- pure . pmatch prevEntryF.next $ \case
        ...
        PDNothing _ ->
          ptraceError
            "newStakeLogic (mintEnd): the previous entry does not point to another entry"
      ---- BUSINESS LOGIC ----
      ...
      pguardC
        "newStakeLogic (mintEnd): spent input is not the same as input in redeemer"
        $ spentInputF.outRef #== entriesRefsF.previousEntry
      -- Previous entry should now point to the new entry
      pguardC
        "newStakeLogic (mintEnd): the previous entry should point to the new entry"
        $ prevEntryUpdatedF.next `pointsTo` newEntryF.key
      -- And new entry should point to the current entry
      pguardC
        "newStakeLogic (mintEnd): the new entry should point to the current entry"
        $ newEntryF.next `pointsTo` currEntryKey
      -- Validate entries' order
      pguardC
        "newStakeLogic (mintEnd): failed to validate order in previous, current and new entry"
    ...
    PMintEnd listEndOutRef' -> unTermCont $ do
      ...
      pguardC
        "newStakeLogic (mintEnd): updated end should point to new end entry"
      ...
      -- Validate entries' order
      pguardC
        "newStakeLogic (mintEnd): new entry's key should come after end entry"
...
withdrawActLogic txInfoF paramsF purpose datum withdrawActParamsF period = do
  ...
  let assetCheck :: Term s PUnit
      ...
      entryCheck entryOutRef = unTermCont $ do
        pguardC
          ...
          $ hasListNft paramsF.assocListCs spentInputResolvedF.value
        pguardC
          "withdrawActLogic: spent entry does not match redeemer TxOutRef"
  ...
  pure . pmatch (pfromData withdrawActParamsF.burningAction) $ \case
    ...
    PBurnSingle entry' -> unTermCont $ do
      ...
      pguardC
        "withdrawActLogic: withdrawn amount does not match stake and rewards"
        $ withdrawnAmt
...
withdrawHeadActLogic spentInputF withdrawnAmt datum txInfoF paramsF period stateOutRef headEntryOutRef =
  unTermCont $ do
    ...
    -- Validate next state
    pguardC
      "withdrawHeadActLogic: next pool state does not point to same location as burned entry"
...
withdrawOtherActLogic spentInputF withdrawnAmt datum txInfoF paramsF period burnEntryOutRef =
  unTermCont $ do
    ...
    pguardC
      "withdrawOtherActLogic: consumed entry key does not match previous entry's key"
      $ prevEntryF.next `pointsTo` burnEntryF.key
    -- Validate updated entry
    pguardC
      "withdrawOtherActLogic: updated previous entry does not point to same location as burned entry"
...
newEntryGuard paramsF newEntryF stakeAmt stakeHolderKey = do
  pguardC
    "newEntryGuard: incorrect init. of newDeposit and deposit fields in first stake"
  ...
  pguardC "newEntryGuard: new entry's stake not within stake bounds" $
    pfromData paramsF.minStake #<= newEntryF.deposited
      #&& pfromData newEntryF.deposited #<= paramsF.maxStake
  pguardC
    "newEntryGuard: new entry's rewards, totalRewards, and totalDeposited fields not initialized to zero"
...
withdrawRewardsGuard ::
  ...
  PEntryHRec s ->
  TermCont s (Term s PUnit)
withdrawRewardsGuard _ amount _ _ entryF =
    pure $
      pif

...
export const main = async () => {
  ...
  let initialIncomplete: IncompleteDeposit = {
    nextDepositAmt: adminDeposit,
    failedKeys: [],
    totalDeposited: { numerator: BigInteger(0), denominator: BigInteger(1)}
  }
  const incompleteDeposit = assocList.entries.reduce(
    (acc, curr) => {
      return {
        nextDepositAmt: acc.nextDepositAmt,
        failedKeys: acc.failedKeys.concat(curr.key),
        totalDeposited: { ...acc.totalDeposited, numerator: acc.totalDeposited.numerator.add(curr.deposited)} }
    },
    initialIncomplete
    );

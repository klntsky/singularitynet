module UnbondedStaking.CreatePool (createUnbondedPoolContract, getUnbondedPoolsContract) where

import Contract.Prelude

import Contract.Address (Bech32String, addressToBech32, getWalletAddress, ownPaymentPubKeyHash, scriptHashAddress)
import Contract.Log (logWarn')
import Contract.Monad (Contract, liftContractM, liftContractM, liftedE, liftedE', liftedM)
import Contract.PlutusData (Datum(Datum), PlutusData, toData)
import Contract.ScriptLookups as ScriptLookups
import Contract.Scripts (validatorHash)
import Contract.Transaction (BalancedSignedTransaction, TransactionHash, TransactionOutputWithRefScript(..), balanceAndSignTx)
...
import Settings (confirmationTimeout, submissionAttempts, unbondedStakingTokenName)
import Types (StakingType(Unbonded))
import Types.Interval (POSIXTime(POSIXTime))
import UnbondedStaking.Types (InitialUnbondedParams(InitialUnbondedParams), UnbondedPoolParams, UnbondedStakingDatum(StateDatum))
import UnbondedStaking.Utils (mkUnbondedPoolParams)
import Utils (addressFromBech32, currentRoundedTime, logInfo_, mustPayToScript, repeatUntilConfirmed)
...
getUnbondedPoolsContract addrStr ibp = do
    ...
    poolUtxos <- liftedM "(getUnbondedPoolsContract) Could not get pool UTxOs" $
       utxosAt =<< addressFromBech32 addrStr
    ...
    stateTokenTn <-
       liftMaybe (Exception.error "(getUnbondedPoolsContract) Could not get bonded staking token name")
          unbondedStakingTokenName
    let getStateTokenCs :: Value -> Maybe CurrencySymbol
        getStateTokenCs =
            Array.head
            <<< map fst
            <<< Array.filter ((_ == stateTokenTn) <<< fst <<< snd)
            <<< flattenValue
        addListTokenCs :: CurrencySymbol -> Contract () (CurrencySymbol /\ CurrencySymbol)
        addListTokenCs stateNftCs = do
            listPolicy <- liftedE (mkListNFTPolicy Unbonded stateNftCs)
            listNftCs <- liftMaybe (Exception.error "Could not obtain currency symbol from list policy") $
               scriptCurrencySymbol listPolicy
            pure $ stateNftCs /\ listNftCs
    symbols <- traverse addListTokenCs $ Array.mapMaybe (getStateTokenCs <<< getValue) $ Array.fromFoldable poolUtxos
    if Array.length symbols > 1
        then logWarn' "(getUnbondedPoolsContract) More than one pool with the given \n\
                      \address"
        else pure unit
    -- For each symbol, we create the bonded params and we returh all of them
    adminPkh <- liftedM "createUnbondedPoolContract: Cannot get admin's pkh"
        ownPaymentPubKeyHash
    pure $ map (\(stateCs /\ listCs) -> mkUnbondedPoolParams adminPkh stateCs listCs ibp) symbols
    where getValue :: TransactionOutputWithRefScript -> Value
